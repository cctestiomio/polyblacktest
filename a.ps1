...
// delta in "$ profit-if-win difference" vs start-of-slug for $stake:
...
export default function BacktestEngine({ sessions }) {
  ...
  // DEFAULT: group price by 1 cent
  const [priceStepCents, setPriceStepCents] = useState(1); // 1,5,10
  ...
  const runBacktest = () => {
    ...
    for (const session of sessions) {
      ...
      // FIX: compute "Price To Beat" as earliest elapsed with a price (not just first array item).
      ...
      for (const point of points) {
        ...
        for (const s of sides) {
          ...
          const d = hasStart ? deltaProfitIfWin(STAKE, startPrice, price) : null;

          // Always compute a display bucket label when possible (even if not grouping by delta).
          ...
          // Keep label stable if grouping-by-delta is OFF; just store the most recent computed label.
          if (b && !normalized.useDelta) {
            cur.deltaBucketStart = b.bs;
            cur.deltaBucketEnd = b.be;
    ...
    const allRows = [...cells.values()]
      ...
      .map(x => {
        ...
        const avgStartPriceLabel = (avgStartPrice01 == null) ? "missing" : fmtPrice01(avgStartPrice01);
              ...
              <BarChart data={(results.chartRows ?? []).slice().reverse()} layout="vertical" margin={{ left: 10, right: 20, top: 10, bottom: 10 }}>
                ...
                <Tooltip
                  formatter={(v, n, ctx) => {
                    ...
                    if (!p) return [v, n];
                    return [
                      `${p.metricLabel}=${p.metricVal}, avgDelta=${p.avgDeltaLabel}, winRate=${p.winRate}%, win/total=${p.winTotal}, realizedPL=$${p.realizedPL}`,
                      "Cell"
                    ];
                  }}
