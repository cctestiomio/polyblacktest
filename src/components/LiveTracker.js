...
const cdRef        = useRef(null);   
      ...
      try {
        ...
        for (const msg of arr) {
          ...
          setCurDown(pricesRef.current.down);
                  const st = slugTsRef.current;
          if (st != null && getSecondsRemaining(st) <= 0 && clockTickFnRef.current) {
            clockTickFnRef.current();
          }
}
      } catch { /* ignore */ }
  ...
  useEffect(() => { clockTickFnRef.current = clockTick; }, [clockTick]);

  const scheduleResolution = useCallback((slugTs) => {
    clearTimeout(resTimeoutRef.current);
    resTimeoutRef.current = null;
    if (slugTs == null) return;
    const ms = (getResolutionTs(slugTs) * 1000) - Date.now();
    const wait = Math.max(0, ms + 25);
    resTimeoutRef.current = setTimeout(() => {
      if (slugTsRef.current !== slugTs) return;
      if (savedRef.current) return;
      if (clockTickFnRef.current) clockTickFnRef.current();
    }, wait);
  }, []);
